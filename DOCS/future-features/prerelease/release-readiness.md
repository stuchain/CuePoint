 ...
 - [x] Define and publish a support policy (supported OS versions, Python
       versions for devs, Rekordbox versions, and expected file formats).
 - [x] Add a clear "first run" onboarding flow or checklist to reduce early
       user errors (input file choice, playlist name, output location).
- [x] Add a guided validation step for inputs before processing starts.
- [x] Provide a clear "cancel + resume later" workflow for long runs.
- [x] Add preflight checks for missing dependencies and permissions.
- [x] Add a "what changed" summary after each run (matches, unmatched, fixes).
- [x] Add a user-facing glossary for matching terms (candidate, confidence).
 
 ## 2) Release Engineering and Distribution
 
 - [x] Verify release workflows are fully automated (tag → build → release →
       appcast/update feed).
 - [x] Add deterministic build inputs (pinned dependencies and hash checking).
 - [x] Record build provenance: commit SHA, build machine, tool versions.
- [ ] Ensure notarization and stapling are documented and verified for macOS.
 - [x] Create a rollback plan and document how to invalidate a bad release.
...
- [x] Add a release gate that blocks if version sync or changelog are missing.
- [x] Add SBOM generation and publish with each release.
- [x] Add reproducible build guidance (toolchain versions, build containers).
...
- [ ] Add tests for update rollback and interrupted update scenarios.
 ...
 - [x] Confirm data handling boundaries (no private user data sent externally
       beyond Beatport search queries).
 - [x] Add a secure telemetry policy (or explicitly no telemetry) and document
       opt-in/out behavior.
 - [x] Add a vulnerability disclosure policy in `docs/security/`.
 - [ ] Run a dependency audit on each release candidate.
 - [x] Validate download/update integrity (checksum/signature verification).
...
- [ ] Add third-party license bundle generation into release artifacts.
 ...
 - [x] Add defensive limits for large XML files (timeouts, memory usage).
 - [x] Add a crash-safe output writer (atomic writes or temp file swap).
 - [x] Add a fail-safe for malformed XML or missing fields.
- [ ] Add a "pause and resume" mechanism for long processing runs.
- [ ] Ensure partial results are preserved on crash or forced exit.
- [x] Add circuit breaker behavior for repeated search failures.
- [ ] Add configurable per-run timeouts and fail-fast options.
- [x] Add resumable processing per playlist or per track batch.
- [x] Add self-healing behavior for stale caches.
- [ ] Ensure all writes happen outside install directories (standard paths).
- [x] Add log rotation and separate crash logs.
