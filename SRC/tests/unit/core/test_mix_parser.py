...
class TestMixParser:
    ...
    def test_extract_generic_parenthetical_phrases(self):
        ...
        assert len(phrases) > 0
        assert any(
            "ivory" in p.lower() or "re-fire" in p.lower() or "refire" in p.lower()
            for p in phrases
        )
...
class TestPhraseMatching:
    ...
    def test_mix_bonus_remixer_match(self):
        ...
        cand_mix = {"is_remix": True, "remixer_tokens": {"camelphat"}}
        bonus, reason = _mix_bonus(input_mix, cand_mix)
        assert bonus > 0
    ...
    def test_mix_bonus_remixer_mismatch(self):
        ...
        input_mix = {"is_remix": True, "remixer_tokens": {"camelphat"}}
        cand_mix = {"is_remix": True, "remixer_tokens": {"other"}}
        bonus, reason = _mix_bonus(input_mix, cand_mix)
        assert bonus < 0
    ...
    def test_extract_remixer_names_from_brackets(self):
        ...
        names = _extract_remixer_names_from_title(
            "Track [CamelPhat & Keinemusik Remix]"
        )
        assert len(names) >= 1
    ...
    def test_merge_name_lists_empty_strings(self):
        ...
        assert "Jane" in result
        assert "Bob" in result
        assert all(p.strip() for p in result.split(",")), (
            "Merged list contains empty names"
        )
