...
class TestShortcutManager(unittest.TestCase):
    ...
    def test_get_shortcut_sequence(self):
        ...
        self.manager.register_shortcut(
            "test_action", "Ctrl+T", lambda: None, ShortcutContext.GLOBAL
        )
    ...
    def test_context_switching(self):
        ...
        self.manager.register_shortcut(
            "global_action", "Ctrl+G", lambda: None, ShortcutContext.GLOBAL
        )

        self.manager.register_shortcut(
            "main_action", "Ctrl+M", lambda: None, ShortcutContext.MAIN_WINDOW
        )
...
class TestShortcutCustomizationDialog(unittest.TestCase):
    ...
    def test_check_conflict(self):
        ...
        if self.dialog.table.rowCount() >= 2:
            ...
            _ = self.dialog.table.item(
                1, 0
            ).text()  # action2 - for conflict check structure
