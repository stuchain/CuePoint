...
class TestBeatportDataIntegration:
    ...
    def test_parse_track_page_malformed_html(self, mock_request):
        ...
        mock_request.return_value = BeautifulSoup(
            "<html><body>Invalid content</body></html>", "html.parser"
        )
    ...
    def test_request_html_beautifulsoup_exception(self, mock_get):
        ...
        with patch(
            "cuepoint.data.beatport.BeautifulSoup", side_effect=Exception("Parse error")
        ):
            result = request_html("https://www.beatport.com/track/test/123")
    ...
    def test_track_urls_direct_search_success(self, mock_direct):
        ...
        mock_direct.return_value = ["https://www.beatport.com/track/test/123456"]

        urls = track_urls(
            1, "Test Track (Remix)", max_results=10, use_direct_search=True
        )
    ...
    def test_track_urls_direct_search_remix_query(self, mock_direct):
        ...
        # Mock direct search returning URLs
        mock_direct.return_value = ["https://www.beatport.com/track/test-remix/123456"]

        urls = track_urls(
            1, "Test Track (Artist Remix)", max_results=10, use_direct_search=None
        )
    ...
    def test_track_urls_site_prefix_extraction(self, mock_direct):
        ...
        urls = track_urls(
            1,
            "site:beatport.com/track Test Track",
            max_results=10,
            use_direct_search=True,
        )
    ...
    def test_track_urls_ddg_search(self):
        ...
        with patch("cuepoint.data.beatport.ddg_track_urls") as mock_ddg:
            mock_ddg.return_value = ["https://www.beatport.com/track/test/123456"]
    ...
    def test_track_urls_original_mix_detection(self, mock_direct):
        ...
        urls = track_urls(
            1, "Test Track Original Mix", max_results=10, use_direct_search=None
        )
    ...
    def test_track_urls_browser_when_direct_empty(self, mock_browser, mock_direct):
        ...
        mock_browser.return_value = ["https://www.beatport.com/track/test/123456"]
    ...
    def test_track_urls_remix_query_few_results(self, mock_direct):
        ...
        with patch.dict(
            "cuepoint.data.beatport.SETTINGS", {"USE_BROWSER_AUTOMATION": False}
        ):
            urls = track_urls(
                1, "Test Track (Remix)", max_results=10, use_direct_search=True
            )
    ...
    def test_ddg_track_urls_extended_mix(self, mock_ddgs_class):
        ...
        mock_result = Mock()
        mock_result.get.return_value = (
            "https://www.beatport.com/track/test-extended/123456"
        )
        mock_ddgs.text.return_value = [mock_result]
        mock_ddgs_class.return_value.__enter__.return_value = mock_ddgs
    ...
    def test_ddg_track_urls_rework_keyword(self, mock_ddgs_class):
        ...
        mock_result = Mock()
        mock_result.get.return_value = (
            "https://www.beatport.com/track/test-rework/123456"
        )
        mock_ddgs.text.return_value = [mock_result]
        mock_ddgs_class.return_value.__enter__.return_value = mock_ddgs
    ...
    def test_track_urls_import_error_fallback(self, mock_direct):
        ...
        with patch(
            "cuepoint.data.beatport.beatport_search_direct",
            ...
            with patch("cuepoint.data.beatport.ddg_track_urls") as mock_ddg:
                ...
                urls = track_urls(
                    1, "Test Track", max_results=10, use_direct_search=True
                )
    ...
    def test_track_urls_browser_fallback_many_ddg_results(self, mock_browser, mock_ddg):
        ...
        mock_ddg.return_value = [
            f"https://www.beatport.com/track/test{i}/12345{i}" for i in range(60)
        ]
    ...
    def test_track_urls_browser_auto_detect_artist_title(self, mock_browser, mock_ddg):
        ...
        with patch.dict(
            ...
            urls = track_urls(
                1, "Test Track Artist Name", max_results=10, use_direct_search=False
            )
    ...
    def test_ddg_track_urls_url_construction_fallback(self, mock_ddgs_class):
        ...
        mock_result = Mock()
        mock_result.get.return_value = (
            "https://www.beatport.com/track/constructed/123456"
        )
        mock_ddgs.text.return_value = [mock_result]
        mock_ddgs_class.return_value.__enter__.return_value = mock_ddgs
    ...
    def test_track_urls_browser_max_results_limit(self, mock_browser, mock_direct):
        ...
        mock_browser.return_value = [
            f"https://www.beatport.com/track/test{i}/12345{i}" for i in range(20)
        ]
    ...
    @patch("cuepoint.data.beatport_search.beatport_search_direct")
    @patch("cuepoint.data.beatport_search.beatport_search_browser")
    def test_track_urls_browser_direct_merge_max_results(
        self, mock_browser, mock_direct
    ):
        """Test track_urls max_results limit when merging browser and direct results."""
        # Mock direct search returning URLs
        mock_direct.return_value = [
            f"https://www.beatport.com/track/direct{i}/12345{i}" for i in range(10)
        ]

        # Mock browser search returning URLs
        mock_browser.return_value = [
            f"https://www.beatport.com/track/browser{i}/12346{i}" for i in range(10)
        ]
        ...
        with patch.dict(
            ...
            urls = track_urls(
                1, "Test Track (Remix)", max_results=5, use_direct_search=True
            )
    ...
    def test_track_urls_browser_ddg_merge_max_results(self, mock_browser, mock_ddg):
        ...
        mock_ddg.return_value = [
            f"https://www.beatport.com/track/ddg{i}/12345{i}" for i in range(60)
        ]
    ...
    def test_track_urls_browser_import_error(self, mock_browser, mock_ddg):
        ...
        with patch(
            "cuepoint.data.beatport.beatport_search_browser",
            side_effect=ImportError("No module"),
        ):
            urls = track_urls(
                1,
    ...
    def test_track_urls_site_prefix_cleaning(self, mock_direct):
        ...
        urls = track_urls(
            1, "site:beatport.com Test Track", max_results=10, use_direct_search=True
        )
    ...
    def test_track_urls_browser_remix_less_than_10(self, mock_browser, mock_direct):
        ...
        mock_direct.return_value = [
            f"https://www.beatport.com/track/test{i}/12345{i}" for i in range(5)
        ]
    ...
    def test_track_urls_browser_merge_direct_first(self, mock_browser, mock_direct):
        ...
        # Mock direct search returning URLs
        mock_direct.return_value = ["https://www.beatport.com/track/direct/123456"]

        # Mock browser search returning URLs
        mock_browser.return_value = ["https://www.beatport.com/track/browser/123457"]
    ...
    @patch("cuepoint.data.beatport_search.beatport_search_direct")
    @patch("cuepoint.data.beatport_search.beatport_search_browser")
    def test_track_urls_browser_merge_max_results_break(
        self, mock_browser, mock_direct
    ):
        """Test track_urls breaking when max_results reached during merge."""
        # Mock direct search returning many URLs
        mock_direct.return_value = [
            f"https://www.beatport.com/track/direct{i}/12345{i}" for i in range(20)
        ]

        # Mock browser search returning many URLs
        mock_browser.return_value = [
            f"https://www.beatport.com/track/browser{i}/12346{i}" for i in range(20)
        ]
    ...
    @patch("cuepoint.data.beatport.ddg_track_urls")
    @patch("cuepoint.data.beatport_search.beatport_search_browser")
    def test_track_urls_browser_ddg_merge_max_results_break(
        self, mock_browser, mock_ddg
    ):
    ...
    def test_ddg_track_urls_trace_logging(self, mock_ddgs_class):
        ...
        with (
            patch("cuepoint.utils.utils.tlog") as mock_tlog,
            patch.dict(
    ...
    def test_ddg_track_urls_exact_remix_query_max_200(self, mock_ddgs_class):
        ...
        if mock_ddgs.text.called:
            ...
            main_search_calls = [
                ...
                if "site:beatport.com/track" in str(call)
                or "site:beatport.com" in str(call)
            ]
            for call in main_search_calls:
                ...
                if "max_results" in kwargs:
                    assert kwargs["max_results"] >= 200, (
                        f"Expected max_results >= 200, got {kwargs['max_results']}"
                    )
                    break  # Found the main search call with increased max_results

    @patch("cuepoint.data.beatport.DDGS")
    @patch("cuepoint.data.beatport.request_html")
    def test_ddg_track_urls_fallback_page_track_url_direct(
        self, mock_request, mock_ddgs_class
    ):
    ...
    @patch("cuepoint.data.beatport.DDGS")
    @patch("cuepoint.data.beatport.request_html")
    def test_ddg_track_urls_fallback_page_parsing_href_continue(
        self, mock_request, mock_ddgs_class
    ):
    ...
    @patch("cuepoint.data.beatport.DDGS")
    @patch("cuepoint.data.beatport.request_html")
    def test_ddg_track_urls_fallback_page_parsing_exception(
        self, mock_request, mock_ddgs_class
    ):
