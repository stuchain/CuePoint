...
class ResultsController:
    ...
    def apply_filters(
        ...
        if search_text:
            ...
            filtered = [
                ...
                if (
                    ...
                    or search_lower in (r.artist or "").lower()
                    or search_lower in (r.beatport_title or "").lower()
                    or search_lower in (r.beatport_artists or "").lower()
                )
            ]
        ...
        if bpm_min is not None or bpm_max is not None:
            filtered = [r for r in filtered if self._bpm_in_range(r, bpm_min, bpm_max)]
    ...
    def _year_in_range(
        self, result: TrackResult, year_min: Optional[int], year_max: Optional[int]
    ) -> bool:
        """
    ...
    def _bpm_in_range(
        self, result: TrackResult, bpm_min: Optional[int], bpm_max: Optional[int]
    ) -> bool:
        """
    ...
    def sort_results(self, key: str, ascending: bool = True) -> List[TrackResult]:
        ...
        if key == "title":
            ...
            self.filtered_results.sort(
                key=lambda r: confidence_order.get(r.confidence, 0), reverse=reverse
            )
        elif key == "year":
            self.filtered_results.sort(key=lambda r: self._get_year(r), reverse=reverse)
        elif key == "bpm":
            self.filtered_results.sort(key=lambda r: self._get_bpm(r), reverse=reverse)
        elif key == "index":
    ...
    def get_batch_summary_statistics(
        self, batch_results: Dict[str, List[TrackResult]]
    ) -> Dict[str, Any]:
        """
