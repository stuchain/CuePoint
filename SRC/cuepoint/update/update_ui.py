...
from cuepoint.update.version_utils import (
    get_version_display_string as format_version_string,
)
...
class UpdateAvailableDialog(QDialog):
    ...
    def __init__(
        self, current_version: str, update_info: Dict, parent: Optional[QWidget] = None
    ):
        """
    ...
    def _setup_ui(self) -> None:
        ...
        # Version information
        new_version = self.update_info.get("short_version") or self.update_info.get(
            "version", "Unknown"
        )
        from cuepoint.version import (
            get_version_display_string as get_app_version_display,
        )
...
class UpdateErrorDialog(QDialog):
    ...
    def __init__(self, error_message: str, parent: Optional[QWidget] = None):
        """
...
def show_update_error_dialog(
    error_message: str, parent: Optional[QWidget] = None
) -> int:
    """
...
class UpdateCheckDialog(QDialog):
    ...
    def __init__(self, current_version: str, parent: Optional[QWidget] = None):
        ...
        try:
            ...
            # Validate parent before passing to super()
                ...
                try:
                    ...
                    if not hasattr(parent, "windowTitle"):
                        logger.warning(
                            "  ✗ Parent does not have windowTitle, using None"
                        )
                        parent = None
                    else:
                        logger.debug(
                            f"  ✓ Parent has windowTitle: '{parent.windowTitle()}'"
                        )
                except (RuntimeError, AttributeError) as parent_error:
                    logger.warning(
                        f"  ✗ Parent widget validation failed: {parent_error}, using None"
                    )
                    parent = None
            else:
            ...
            self._download_url: Optional[str] = (
                None  # Store download URL for context menu
            )
        ...
        try:
            ...
            try:
                ...
                current_version_label = QLabel(
                    f"Version: {get_version_display_string()}"
                )
            ...
            self.results_label.setOpenExternalLinks(
                False
            )  # We'll handle links manually for copy support
            self.results_label.setContextMenuPolicy(
                Qt.ContextMenuPolicy.CustomContextMenu
            )
            self.results_label.customContextMenuRequested.connect(
                self._on_results_label_context_menu
            )
            ...
            self.download_button.customContextMenuRequested.connect(
                self._on_download_button_context_menu
            )
            ...
            try:
                ...
                logger.error(
                    f"Could not even create fallback UI: {fallback_error}",
                    exc_info=True,
                )
                raise
    ...
    def set_update_found(self, update_info: Dict) -> None:
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"set_update_found called with version: {update_info.get('short_version', 'unknown')}, download_url: {update_info.get('download_url', 'none')}"
        )

        self.progress_bar.setVisible(False)

        new_version = update_info.get("short_version") or update_info.get(
            "version", "Unknown"
        )
        from cuepoint.version import (
            get_version_display_string as get_app_version_display,
        ...
        if release_notes_url:
            results_html += (
                f'<a href="{release_notes_url}">View Release Notes</a><br><br>'
            )
        ...
        logger.info(
            f"Download button state - visible: {self.download_button.isVisible()}, enabled: {self.download_button.isEnabled()}, text: {self.download_button.text()}"
        )
        ...
        if parent and hasattr(parent, "_on_update_install_from_dialog"):
            ...
            try:
                ...
                def fallback_handler():
                    ...
                    btn_logger.info("=" * 70)
                    btn_logger.info(
                        "This means the main connection failed, using fallback"
                    )
                    try:
                        parent_window._on_update_install_from_dialog()
                    except Exception as e:
                        btn_logger.error(
                            f"Error in fallback handler: {e}", exc_info=True
                        )
                        import traceback
    ...
    def _show_button_diagnostics(self) -> None:
        ...
        try:
            diagnostics.append("\n✓ Connection status:")
            diagnostics.append(
                f"  - Has _download_connected: {hasattr(self, '_download_connected')}"
            )
            if hasattr(self, "_download_connected"):
                diagnostics.append(
                    f"  - _download_connected value: {self._download_connected}"
                )
            else:
        ...
        try:
            ...
            if hasattr(self, "update_info"):
                if self.update_info:
                    ...
                    diagnostics.append(
                        f"    Version: {self.update_info.get('short_version', 'N/A')}"
                    )
                    diagnostics.append(
                        f"    Download URL: {self.update_info.get('download_url', 'N/A')[:80]}..."
                    )
                else:
        ...
        try:
            ...
            if parent:
                ...
                diagnostics.append(
                    f"  - Has _on_update_install_from_dialog: {hasattr(parent, '_on_update_install_from_dialog')}"
                )
                ...
                if hasattr(parent, "update_check_dialog"):
                    ...
                    diagnostics.append(
                        "  - ✗ Parent missing update_check_dialog reference"
                    )
            else:
        ...
        try:
            ...
            if parent and hasattr(parent, "update_manager"):
                ...
                if parent.update_manager:
                    ...
                    diagnostics.append(
                        f"  - Has _update_available: {hasattr(parent.update_manager, '_update_available')}"
                    )
                    if hasattr(parent.update_manager, "_update_available"):
                        ...
                        if fallback:
                            ...
                            diagnostics.append(
                                f"    Version: {fallback.get('short_version', 'N/A')}"
                            )
                        else:
...
def show_update_check_dialog(
    current_version: str, parent: Optional[QWidget] = None
) -> UpdateCheckDialog:
    """
    ...
    try:
        ...
        if sys.platform == "darwin":
            ...
            try:
                ...
                logger.warning(
                    f"  ✗ Parent widget is invalid: {parent_error}, creating dialog without parent"
                )
                parent = None
        else:
        ...
        try:
            ...
            logger.error(
                f"  ✗ Error creating UpdateCheckDialog: {create_error}", exc_info=True
            )
            ...
            try:
                ...
                logger.error(
                    f"  ✗ Error creating UpdateCheckDialog even without parent: {fallback_error}",
                    exc_info=True,
                )
                raise
        ...
        try:
            ...
            logger.error(
                f"  ✗ Error showing update check dialog: {show_error}", exc_info=True
            )
        ...
        try:
            ...
            class MinimalUpdateDialog:
                ...
                def set_update_found(self, info):
                    pass
            ...
            logger.error(
                f"Could not even create minimal dialog: {minimal_error}", exc_info=True
            )
            raise
