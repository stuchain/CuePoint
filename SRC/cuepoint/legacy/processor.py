...
Key functions:
- process_track(): Processes a single track (generates queries, finds match,
  returns results)
...
# Async I/O imports are already handled above
...
def generate_summary_report(
    ...
    review_count = sum(
        1
        for r in rows
        if float(r.get("match_score", "0") or 0) < 70
        or int(float(r.get("artist_sim", "0") or 0)) < 50
        or not (r.get("beatport_url") or "").strip()
    )
    ...
    matched_title_sims = [
        int(float(r.get("title_sim", "0") or 0))
        for r in rows
        if (r.get("beatport_url") or "").strip()
    ]
    _ = sum(matched_title_sims) / len(matched_title_sims) if matched_title_sims else 0

    matched_artist_sims = [
        int(float(r.get("artist_sim", "0") or 0))
        for r in rows
        if (r.get("beatport_url") or "").strip()
    ]
    ...
    lines.append(
        "| "
        + f"  [FAIL] Unmatched: {unmatched} ({100 - match_rate:.1f}%){' ' * unmatched_padding}"
        + "|"
    )
    review_pct = review_count / total_tracks * 100
    review_padding = 78 - 37 - len(str(review_count)) - len(f"{review_pct:.1f}")
    lines.append(
        "| "
        + f"  [REVIEW] Review Needed: {review_count} ({review_pct:.1f}%){' ' * review_padding}"
        + "|"
    )
    lines.append("+" + "=" * 78 + "+")
    lines.append("| " + "Match Quality:" + " " * 63 + "|")
    high_conf_pct = high_conf / total_tracks * 100
    high_conf_padding = 78 - 39 - len(str(high_conf)) - len(f"{high_conf_pct:.1f}")
    lines.append(
        "| "
        + f"  High Confidence (>=90): {high_conf} ({high_conf_pct:.1f}%){' ' * high_conf_padding}"
        + "|"
    )
    med_conf_pct = med_conf / total_tracks * 100
    med_conf_padding = 78 - 42 - len(str(med_conf)) - len(f"{med_conf_pct:.1f}")
    lines.append(
        "| "
        + f"  Medium Confidence (70-89): {med_conf} ({med_conf_pct:.1f}%){' ' * med_conf_padding}"
        + "|"
    )
    low_conf_pct = low_conf / total_tracks * 100
    low_conf_padding = 78 - 36 - len(str(low_conf)) - len(f"{low_conf_pct:.1f}")
    lines.append(
        "| "
        + f"  Low Confidence (<70): {low_conf} ({low_conf_pct:.1f}%){' ' * low_conf_padding}"
        + "|"
    )
    ...
    lines.append(
        "| "
        + f"  Early Exits: {early_exits} ({early_exit_str}% of tracks){' ' * early_exit_padding}"
        + "|"
    )
...
def process_track(
    ...
    Args:
        ...
        track: Track object containing original track data from Rekordbox
    ...
    # Extract original artist information from track
    ...
    try:
        _logger.info(
            f"[{idx}] Searching Beatport for: {clean_title_for_log} - "
            f"{original_artists or artists_for_scoring}"
        )
    ...
    if best and best.score >= SETTINGS["MIN_ACCEPT_SCORE"]:
        ...
        try:
            _logger.info(
                f"[{idx}] -> Match: {best.title} - {best.artists} "
        ...
        _logger.info(
            f"[{idx}] -> Match: {safe_title} - {safe_artists} "
...
def process_track_with_callback(
    ...
    min_accept_score = effective_settings.get(
        "MIN_ACCEPT_SCORE", SETTINGS.get("MIN_ACCEPT_SCORE", 70)
    )
    ...
    if best and best.score >= min_accept_score:
        ...
        return TrackResult(
            ...
            beatport_track_id=beatport_track_id,
            candidates_data=cand_rows,
            queries_data=queries_rows,
            search_query_index=str(best.query_index),
            search_stop_query_index=str(stop_qidx),
            candidate_index=str(best.candidate_index),
        )
...
def process_playlist(
    ...
    It supports progress callbacks, cancellation, and both sequential and parallel
    processing.
    ...
    # Use track.position if available, otherwise use enumerate index
    inputs: List[Tuple[int, Track]] = []
    for track in playlist.tracks:
        idx = track.position if track.position is not None else len(inputs) + 1
        inputs.append((idx, track))
    ...
    # Create a mapping for quick lookup (used when tracking unmatched tracks
    # in parallel mode)
    inputs_map = {idx: track for idx, track in inputs}
        ...
        with ThreadPoolExecutor(max_workers=track_workers) as ex:
            ...
            future_to_args = {
                ex.submit(
                    ...
                    progress_callback=None,  # Don't pass callback to individual
                    # tracks in parallel mode
            ...
            results_dict: Dict[int, TrackResult] = {}  # Store results by index
            # for ordering
    ...
    if auto_research and not (controller and controller.is_cancelled()):
        ...
        if unmatched_results:
            ...
            unmatched_inputs = [
                (result.playlist_index, inputs_map[result.playlist_index])
            ...
            if unmatched_inputs:
                ...
                if track_workers > 1 and len(unmatched_inputs) > 1:
                    # Parallel re-search
                    _logger.info(
                        f"\nRe-searching {len(unmatched_inputs)} unmatched tracks "
                        f"using parallel processing with "
                        f"{min(track_workers, len(unmatched_inputs))} workers"
                    )
                    ...
                    reason = (
                        "only 1 unmatched track"
                        if len(unmatched_inputs) == 1
                        else f"TRACK_WORKERS={track_workers}"
                    )
                    _logger.info(
                        f"\nRe-searching {len(unmatched_inputs)} unmatched tracks "
                        f"using sequential processing ({reason})"
                    )
                    for idx, track in unmatched_inputs:
                        if controller and controller.is_cancelled():
                            break
...
def run(
    ...
    This function maintains the existing CLI interface while using the new
    GUI-ready backend.
        ...
        pbar.set_postfix(
            {
                ...
                "current": (
                    ...
                    else f"Track {progress_info.completed_tracks}"
                ),
            }
        )
            ...
            try:
                ...
                def cli_progress_callback_research(progress_info: ProgressInfo):
                    ...
                    if pbar_research_container[0] is None:
                        pbar_research_container[0] = tqdm(
                            total=progress_info.total_tracks,
                            desc="Re-searching",
                            unit="track",
                            bar_format=(
                ...
                new_matches = sum(
                    ...
                    if r.matched
                    and (
                        results_dict.get(r.playlist_index) is None
                        or not results_dict.get(r.playlist_index).matched
                    )
                )
                if new_matches > 0:
                    _logger.info(f"\n{'=' * 80}")
                    _logger.info(f"Found {new_matches} new match(es)!")
                    _logger.info("Updated CSV files.")
                    print(f"\n{'=' * 80}")
    ...
    summary_file = os.path.join(
        output_dir,
        (
            re.sub(r"\.csv$", "_summary.txt", base_filename)
            if base_filename.lower().endswith(".csv")
            else base_filename + "_summary.txt"
        ),
    )
    try:
...
def process_track_async(
    ...
    if not ASYNC_AVAILABLE or not ASYNC_MATCHER_AVAILABLE:
        ...
        return process_track_with_callback(idx, track, settings)
            ...
            async def process_batch():
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    for idx, track in batch:
                        ...
                        # Create async task for each track using a factory function to capture
                        # variables
                        def create_task(track_idx, track_obj):
                            async def process_single_track():
                                ...
                                if not original_artists.strip():
                                    ex = extract_artists_from_title(track_obj.title)
                                    if ex:
                                        artists_for_scoring, extracted_title = ex
                                ...
                                queries = make_search_queries(
                                    title_for_search,
                                    ("" if title_only_search else artists_for_scoring),
                                    original_title=track_obj.title,
                                )
                        ...
                        tasks.append(create_task(idx, track)())
