...
class ErrorReporter:
    ...
    def __init__(
        self, github_repo: str, github_token: Optional[str] = None, enabled: bool = True
    ):
        """
            ...
            if self._cache_file.exists():
                with open(self._cache_file, "r", encoding="utf-8") as f:
                    ...
                    for error_hash, timestamp_str in data.items():
                        try:
                            self._reported_errors[error_hash] = datetime.fromisoformat(
                                timestamp_str
                            )
    ...
    def report_error(
        ...
        if not self._should_report_error(error_hash):
            self.logger.debug(
                f"Error already reported recently, skipping: {error_hash}"
            )
            return None
...
def init_error_reporter(
    github_repo: str, github_token: Optional[str] = None, enabled: bool = True
) -> None:
