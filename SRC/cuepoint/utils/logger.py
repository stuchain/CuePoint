...
class CuePointLogger:
    ...
    def configure(
        ...
        if build_info.get("commit_sha"):
            logger.info(
                f"Commit: {build_info.get('short_commit_sha', build_info['commit_sha'][:7])}"
            )
...
class CrashLogger:
    ...
    @staticmethod
    def write_crash_info(
        crash_log: Path, exception: Exception, traceback_str: str
    ) -> None:
    ...
    def sanitize_message(message: str) -> str:
        ...
        for url_match in re.findall(url_pattern, sanitized):
            sanitized = sanitized.replace(
                url_match, LogSanitizer.sanitize_url(url_match)
            )
