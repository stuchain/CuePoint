...
from cuepoint.core.query_generator import _artist_tokens
...
def best_beatport_match(
    ...
    # parsed_cache: URL -> (title, artists, key, year, bpm, label,
    #                       genres, release_name, release_date)
    parsed_cache: Dict[
        str,
        Tuple[
            str,
    ...
    def consider(
        ...
        t_sim, a_sim, comp = score_components(
            track_title, track_artists_for_scoring, title, artists or ""
        )
        ...
        if a_sim >= 95:  # Very high artist similarity (95%+)
            if input_mix and input_mix.get("is_remix") and cand_mix.get("is_remix"):
                ...
                # Don't boost non-remix matches for remix queries unless
                # title similarity is reasonable
                pass
        ...
        if track_artists_for_scoring and artists:
            ...
            if len(input_artist_tokens) > 0:
                ...
                # BUT: Only if artist similarity is very low (< 30), and reduce penalty
                # to avoid rejecting valid matches
                elif t_sim >= 95 and overlap_count == 0 and a_sim < 30:
                    # Exact title but completely wrong artist with very low
                    # similarity - moderate penalty
                    final -= 15
        ...
        if title_only_mode:
            ...
            if is_remix_query:
                ...
                # Example: "Tighter (CamelPhat Remix)" vs
                # "Tighter (feat. Jalja) CamelPhat Extended Remix"
    ...
    for i, q in enumerate(queries, 1):
        ...
        if (
            not SETTINGS.get("RUN_ALL_QUERIES")
            and budget
            and elapsed > budget
            and i > min_priority_queries
        ):
            vlog(idx, "[timeout] per-track budget exceeded")
            break

        remix_like_intent = bool(
            (input_mix and input_mix.get("is_remix")) or (input_generic_phrases)
        )
        ...
        def fetch(
            ...
            Returns:
                Tuple of (url, title, artists, key, year, bpm, label, genres,
                         release_name, release_date, elapsed_ms).
            """
        ...
        if (not SETTINGS.get("RUN_ALL_QUERIES")) and best and best.guard_ok:
            ...
            if SETTINGS.get("EARLY_EXIT_SCORE") and best.score >= SETTINGS["EARLY_EXIT_SCORE"]:
                ...
                if i >= int(min_q_for_exit or 0):
                    ...
                    mix_ok = (
                        not SETTINGS.get("EARLY_EXIT_REQUIRE_MIX_OK", True)
                    ) or _mix_ok_for_early_exit(
