...
def beatport_search_direct(idx: int, query: str, max_results: int = 50) -> List[str]:
    ...
    try:
        ...
        vlog(idx, f"[beatport-direct] Searching: {search_url}")  # noqa: F541
        ...
        for script in soup.find_all("script"):
            if script.string:
                ...
                matches = re.findall(
                    r'(https?://[^"\s]+beatport\.com/track/[^"\s]+/\d+)', script.string
                )
...
def beatport_search_browser(idx: int, query: str, max_results: int = 50) -> List[str]:
    ...
    try:
        ...
        vlog(
            idx,
            "[beatport-browser] Selenium not available - install with: pip install selenium",
        )
    except Exception as e:
...
def beatport_search_hybrid(
    idx: int, query: str, max_results: int = 50, prefer_direct: bool = True
) -> List[str]:
