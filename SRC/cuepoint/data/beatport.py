...
def beatport_search_direct(idx: int, query: str, max_results: int) -> List[str]:
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.warning(
            f"[{idx}] Direct Beatport search failed for query={query!r}: {e!r}",
            exc_info=True,
        )
...
def beatport_search_browser(idx: int, query: str, max_results: int) -> List[str]:
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.warning(
            f"[{idx}] Browser automation failed for query={query!r}: {e!r}",
            exc_info=True,
        )
...
def request_html(url: str) -> Optional[BeautifulSoup]:
    ...
    Returns:
        BeautifulSoup object if successful, None if the request failed
        after all retries.
    ...
    def _get(
        u: str, headers: Optional[Dict[str, str]] = None
        ...
        try:
            ...
            if resp:
                ...
                # (requests_cache adds this attribute)
    ...
    if _is_empty_body(resp):
        ...
        vlog(
            "fetch",
            f"[http] empty body (enc={enc}, cl={cl}); retry identity",
        )
    ...
    for _stale_retry in range(2):
        ...
        if not artists:
            ...
            if header:
                ...
                names = [
                    c.get_text(strip=True) for c in chips if c.get_text(strip=True)
                ]
                if names:
        ...
        date_str = info.get("release_date") or val_after_label(
            r"(Release Date|Released)"
        )
        rel_date_iso = None
        year = None
        if date_str:
        ...
        if (
            (not result[0] and not result[1])
            and get_last_cache_hit()
            and _stale_retry == 0
        ):
...
def track_urls(
    ...
    if use_direct_search is None:
        ...
        # Clean query for detection - remove quotes but preserve parentheses
        # (they indicate remix info)
    ...
    if use_direct_search:
        ...
        try:
            ...
            _diag(
                f"[search] direct search -> {len(direct_urls)} urls (max_results={max_results})"
            )
            ...
            # If direct search found results, check if we need browser automation
            # for remix queries
            if direct_urls and len(direct_urls) > 0:
                # If we found some results but it's a remix query and we found
                # very few, try browser automation
                if (
                    is_remix_query
                    ...
                    vlog(
                        idx,
                        f"[search] Direct search found {len(direct_urls)} URLs for "
                        f"remix query, trying browser automation",
                    )
                    ...
                    if browser_urls:
                        _diag(
                            f"[search] browser automation -> {len(browser_urls)} urls"
                        )
                        ...
                        vlog(
                            idx,
                            f"[search] Combined {len(merged)} URLs "
            ...
            # Method 2: Try browser automation if enabled and direct search found
            # nothing OR very few results
            # For remix queries, browser automation is critical since pages are
            # JS-rendered
            # CRITICAL: If direct search found very few results (<10) for a remix
            # query, try browser automation
            # This fixes cases like "Tighter (CamelPhat Remix) HOSH" where direct
            # search might miss the track
            if SETTINGS.get("USE_BROWSER_AUTOMATION", False):
                # If direct search found nothing, or if it's a remix query and
                # found very few results, try browser
                ...
                if direct_urls is None or len(direct_urls) == 0:
                    ...
                    # For remix queries, if we found <10 results, browser might
                    # find more
                    vlog(
                        idx,
                        f"[search] Direct search found only {len(direct_urls)} URLs "
                        f"for remix query, trying browser automation",
                    )
                    should_try_browser = True

                if should_try_browser:
                    # Use the cleaned search_query (without site: prefix) for
                    # browser automation
                    browser_query = (
                    ...
                    if browser_urls:
                        ...
                        # If direct search also found some, merge them
                        # (browser results first, they're more reliable)
                        if direct_urls and len(direct_urls) > 0:
                            ...
                            vlog(
                                idx,
                                f"[search] Merged {len(merged)} URLs "
                                f"(browser {len(browser_urls)} + direct {len(direct_urls)})",
                            )
                            return merged[:max_results]
                        return browser_urls
    ...
    _diag(
        f"[search] ddg_track_urls -> {len(ddg_urls)} urls (max_results={max_results})"
    )
    ...
    if not ddg_urls and not use_direct_search:
        try:
            ...
            _diag(
                f"[search] ddg empty -> direct fallback -> {len(direct_fallback)} urls"
            )
            if direct_fallback:
                return direct_fallback[:max_results]
            if SETTINGS.get("USE_BROWSER_AUTOMATION", False):
                browser_fallback = beatport_search_browser(
                    idx, fallback_query, max_results
                )
                _diag(
                    f"[search] ddg empty -> browser fallback -> {len(browser_fallback)} urls"
                )
                if browser_fallback:
    ...
    if should_try_browser and SETTINGS.get("USE_BROWSER_AUTOMATION", False):
        vlog(
            idx,
            f"[search] DDG found {len(ddg_urls)} results, trying browser automation "
        ...
        try:
            ...
            if browser_urls:
                ...
                # (browser is more reliable, prepend its results)
                seen = set()
                merged = []
                # First add browser results (more reliable)
                for url in browser_urls:
...
def ddg_track_urls(idx: int, query: str, max_results: int) -> List[str]:
    ...
    if SETTINGS.get("DDG_PREFLIGHT_ENABLED", True):
        ...
        try:
            ...
            raw_sock = socket.create_connection(
                ("duckduckgo.com", 443), timeout=preflight_timeout
            )
            try:
    ...
    try:
            ...
            for search_q in search_queries:
                try:
                    ...
                    try:
                        ...
                        is_ddgs_exception = (
                            "DDGSException" in str(type(e).__name__)
                            or "IndexError" in str(e)
                            or "list index out of range" in str(e)
                        )
                        is_timeout = (
                            "TimeoutException" in str(type(e).__name__)
                    ...
                    if is_timeout:
                        ...
                        emsg = str(e).lower()
                        if (
                            "handshake operation timed out" in emsg
                            or "connecttimeout" in emsg
                        ):
                            break
                        ...
                        vlog(
                            idx,
                            f"[search] ddgs parsing error (known issue, will use fallback): {e!r}",
                        )
                    else:
        ...
        logger.error(
            f"[{idx}] DuckDuckGo search (ddgs) not available: {e!r}", exc_info=True
        )
    ...
    try:
        ...
        if (
            ...
            and (" " in ql)
            and (not timed_out)
            and SETTINGS.get("DDG_ENABLED", True)
        ):
            ...
            try:
                with DDGS(
                    proxy=ddg_proxy, timeout=ddg_timeout, verify=ddg_verify
                ) as ddgs:
                    for fallback_q in fallback_queries:
                        try:
                            for r in ddgs.text(
                                fallback_q, region=ddg_region, max_results=20
                            ):
                                href = r.get("href") or r.get("url") or ""
                            ...
                            logger.debug(
                                f"[{idx}] Fallback search error for '{fallback_q}': {e!r}"
                            )
                            vlog(
                                idx,
                                f"[fallback-search] error for '{fallback_q}': {e!r}",
                            )
                            continue
            except Exception as e:
