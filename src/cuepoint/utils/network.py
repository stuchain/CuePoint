...
class RetryCallback(Protocol):
    ...
    def on_retry(
        self, attempt: int, max_attempts: int, delay: float, error: Exception
    ) -> None:
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            for attempt in range(retry_config.max_retries + 1):
                try:
                    ...
                    if tracker:
                        tracker.on_retry(
                            attempt + 1, retry_config.max_retries, total_delay, e
                        )
    ...
    def is_online(force_check: bool = False) -> bool:
        ...
        if not force_check and NetworkState._is_online is not None:
            if NetworkState._last_check is not None:
                if (
                    time.time() - NetworkState._last_check
                    < NetworkState._check_interval
                ):
