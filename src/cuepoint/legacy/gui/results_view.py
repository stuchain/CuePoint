...
class ResultsView(QWidget):
    ...
    def init_ui(self):
        ...
        advanced_filters_group.toggled.connect(
            self.advanced_filters_container.setVisible
        )
    ...
    def _populate_table(self):
        ...
        max_index = (
            max((r.playlist_index for r in filtered), default=1) if filtered else 1
        )
    ...
    def _filter_results(self) -> List[TrackResult]:
        ...
        confidence = (
            self.confidence_filter.currentText()
            if self.confidence_filter.currentText() != "All"
            else None
        )
    ...
    def _show_context_menu_for_table(
        self, position, table: QTableWidget, results: List[TrackResult]
    ):
    ...
    def _on_row_double_clicked_for_table(
        self, index, table: QTableWidget, results: List[TrackResult]
    ):
    ...
    def export_all_csv(self):
        ...
        try:
            ...
            # Write all CSV files
            output_files = write_csv_files(
                self.results, timestamped_filename, output_dir
            )
