...
class HistoryView(QWidget):
            ...
            with open(candidates_path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    if (
                        ...
                        and row.get("original_title", "").strip()
                        == original_title.strip()
                        and row.get("original_artists", "").strip()
                        == original_artists.strip()
                    ):
                        ...
                        candidate = {
                            ...
                            "match_score": float(row.get("final_score", 0))
                            if row.get("final_score")
                            else 0.0,
                            "title_sim": float(row.get("title_sim", 0))
                            if row.get("title_sim")
                            else 0.0,
                            "artist_sim": float(row.get("artist_sim", 0))
    ...
    def _on_row_double_clicked(self, index):
        ...
        playlist_index_item = self.table.item(
            row, 0
        )  # Assuming first column is playlist_index
