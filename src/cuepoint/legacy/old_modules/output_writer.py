            ...
            if result.matched and result.beatport_url:
                ...
                if include_metadata:
                    ...
                    track_data["match"]["scores"] = {
                        ...
                        "title_sim": float(result.title_sim)
                        if result.title_sim is not None
                        else None,
                        "artist_sim": float(result.artist_sim)
...
def write_excel_file(
    ...
    for row_idx, result in enumerate(results, start=2):
        ...
        ws.cell(
            row=row_idx,
            column=11,
            value=int(result.beatport_year)
            if result.beatport_year and result.beatport_year.isdigit()
            else None,
        )
