    ...
    def _install_windows(self, installer_path: Path) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            try:
                ...
                app_path = Path(
                    os.path.expandvars(r"%LOCALAPPDATA%\CuePoint\CuePoint.exe")
                )
            ...
            if launcher_ps1 and launcher_ps1.exists():
                ...
                app_path_str = (
                    str(app_path.resolve()) if app_path and app_path.exists() else ""
                )
                ...
                if not installer_path_str or installer_path_str == "":
                    ...
                    try:
                        ...
                        error_msg = (
                            f"Failed to launch PowerShell script: {str(launch_error)}"
                        )
            ...
            if not launcher_ps1 or not launcher_ps1.exists():
                ...
                try:
                    ...
                    if process.poll() is not None:
                        return_code = process.returncode
                        error_msg = (
                            f"Installer exited immediately with code {return_code}"
                        )
                        logger.error(error_msg)
                        return False, error_msg
