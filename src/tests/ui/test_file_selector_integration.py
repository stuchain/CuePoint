...
def test_integration():
    ...
    try:
        ...
        assert hasattr(window.file_selector, "browse_file"), (
            "browse_file method missing"
        )
        assert hasattr(window.file_selector, "validate_file"), (
            "validate_file method missing"
        )
        assert hasattr(window.file_selector, "get_file_path"), (
            "get_file_path method missing"
        )
        assert hasattr(window.file_selector, "set_file"), "set_file method missing"
        print("   [OK] All FileSelector methods present")

        # Check signal exists
        assert hasattr(window.file_selector, "file_selected"), (
            "file_selected signal missing"
        )
        ...
        print(
            f"   [OK] validate_file works (test.xml exists: {os.path.exists(test_path)})"
        )
