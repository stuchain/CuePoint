...
class TestExportServiceIntegration:
    ...
    def test_export_to_csv_integration(self, export_service, sample_results):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert any(f.stat().st_size > 0 for f in csv_files), (
                "At least one CSV file should not be empty"
            )
    ...
    def test_export_to_json_special_characters(self, export_service):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                assert "Test" in data[0].get("original_title", "") or "Test" in str(
                    data[0]
                )
    ...
    def test_export_to_excel_missing_dependency(self, export_service, sample_results):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(builtins, "__import__", side_effect=mock_import):
                ...
                assert (
                    "EXPORT_EXCEL_MISSING_DEPENDENCY" in str(exc_info.value.error_code)
                    or "openpyxl" in str(exc_info.value.message).lower()
                )
