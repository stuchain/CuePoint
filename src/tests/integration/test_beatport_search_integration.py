...
class TestBeatportSearchIntegration:
    ...
    def test_beatport_search_direct_api_success(self, mock_request, mock_api):
        ...
        mock_api.return_value = ["https://www.beatport.com/track/test/123456"]
    ...
    def test_beatport_search_hybrid_combines_results(
        self, mock_track_urls, mock_direct
    ):
    ...
    def test_beatport_search_hybrid_max_results(self, mock_track_urls, mock_direct):
        ...
        mock_track_urls.return_value = [
            f"https://www.beatport.com/track/another-{i}/12346{i}" for i in range(20)
        ]
        ...
        data = {
            "props": {
                "pageProps": {
                    "dehydratedState": {
                        "queries": [
                            {"state": {"data": [{"id": 123456, "slug": "test-track"}]}}
                        ]
                    }
                }
            }
    ...
    def test_extract_track_ids_recursive_traversal(self):
        ...
        data = {"level1": {"level2": {"level3": {"id": 123456, "slug": "test-track"}}}}
    ...
    def test_extract_track_ids_url_fields(self):
        ...
        data = {
            "track1": {"url": "https://www.beatport.com/track/test/123456"},
            "track2": {"href": "https://www.beatport.com/track/another/123457"},
            "track3": {"link": "https://www.beatport.com/track/third/123458"},
            "track4": {"slug": "test-slug", "id": 123459},
        }
    ...
    def test_beatport_search_direct_max_results_break(self, mock_request, mock_api):
        ...
        links = "\n".join(
            [
                f'<a href="/track/track-{i}/{123456 + i}">Track {i}</a>'
                for i in range(20)
            ]
        )
        html_content = f"""
    ...
    def test_beatport_search_hybrid_prefer_ddg(self, mock_track_urls, mock_direct):
        ...
        # Mock DuckDuckGo returning URLs
        mock_track_urls.return_value = ["https://www.beatport.com/track/test/123456"]

        # Mock direct search returning URLs
        mock_direct.return_value = ["https://www.beatport.com/track/another/123457"]

        urls = beatport_search_hybrid(
            1, "Test Track", max_results=10, prefer_direct=False
        )
    ...
    def test_beatport_search_browser_playwright_success(self):
        ...
        with patch.dict(
            ...
            with patch("cuepoint.data.beatport_search._PLAYWRIGHT_USABLE", True):
    ...
    def test_beatport_search_browser_playwright_exception(self, mock_import):
        ...
        mock_playwright.sync_api.sync_playwright = Mock(
    ...
    def test_beatport_search_browser_selenium_exception(self, mock_import):
        ...
        with patch(
            "selenium.webdriver.Chrome", side_effect=Exception("Selenium error")
        ):
            urls = beatport_search_browser(1, "Test Track", max_results=10)
    ...
    @patch("cuepoint.data.beatport_search.beatport_search_via_api")
    @patch("cuepoint.data.beatport_search.request_html")
    def test_beatport_search_direct_script_regex_max_results_345(
        self, mock_request, mock_api
    ):
    ...
    @patch("cuepoint.data.beatport_search.beatport_search_via_api")
    @patch("cuepoint.data.beatport_search.request_html")
    def test_beatport_search_direct_relative_path_max_results_357(
        self, mock_request, mock_api
    ):
        ...
        paths_in_script = " ".join(
            [f'"/track/test-{i}/{12345 + i}"' for i in range(20)]
        )
        html_content = f"""
        <html>
        <body>
    ...
    @patch("cuepoint.data.beatport_search.beatport_search_via_api")
    @patch("cuepoint.data.beatport_search.request_html")
    def test_beatport_search_direct_data_attributes_max_results_372(
        self, mock_request, mock_api
    ):
        ...
        divs = "\n".join(
            [
                f'<div data-track-id="{12345 + i}" data-track-slug="test-{i}">Track {i}</div>'
                for i in range(20)
            ]
        )
        html_content = f"""
