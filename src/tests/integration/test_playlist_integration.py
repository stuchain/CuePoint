...
def test_integration():
    ...
    try:
        ...
        assert hasattr(window, "playlist_selector"), (
            "PlaylistSelector not found in MainWindow"
        )
        print("   [OK] PlaylistSelector integrated")

        # Check PlaylistSelector methods
        assert hasattr(window.playlist_selector, "load_xml_file"), (
            "load_xml_file method missing"
        )
        assert hasattr(window.playlist_selector, "get_selected_playlist"), (
            "get_selected_playlist method missing"
        )
        assert hasattr(window.playlist_selector, "get_playlist_track_count"), (
            "get_playlist_track_count method missing"
        )
        assert hasattr(window.playlist_selector, "clear"), "clear method missing"
        print("   [OK] All PlaylistSelector methods present")

        # Check signal exists
        assert hasattr(window.playlist_selector, "playlist_selected"), (
            "playlist_selected signal missing"
        )
        print("   [OK] playlist_selected signal exists")

        # Check signal connection
        assert hasattr(window, "on_playlist_selected"), (
            "on_playlist_selected handler missing"
        )
        ...
        print("=" * 60)
        print("\nTo test PlaylistSelector standalone:")
