...
from cuepoint.utils.error_reporter import (
    ErrorReporter,
    init_error_reporter,
    report_error,
)
...
class TestErrorReporter:
    ...
    def error_reporter(self, temp_cache_dir, monkeypatch):
        ...
        reporter = ErrorReporter(
            github_repo="test/repo", github_token="test_token", enabled=True
        )
    ...
    def test_should_report_error_old_error(self, error_reporter):
        ...
        error_reporter._reported_errors["old_error"] = datetime.now() - timedelta(
            hours=25
        )
    ...
    def test_report_error_success(
        self, mock_build_info, mock_version, mock_post, error_reporter, temp_cache_dir
    ):
    ...
    @patch("cuepoint.utils.error_reporter.get_version")
    @patch("cuepoint.utils.error_reporter.get_build_info")
    def test_report_error_with_build_info(
        self, mock_build_info, mock_version, mock_post, error_reporter
    ):
