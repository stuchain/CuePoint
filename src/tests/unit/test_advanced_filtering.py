...
class TestAdvancedFiltering(unittest.TestCase):
    ...
    def test_year_range_filter_min_only(self):
        ...
        years = [
            int(r.beatport_year) for r in filtered if r.matched and r.beatport_year
        ]
...
class TestHistoryViewFiltering(unittest.TestCase):
    ...
    def test_bpm_range_filter_min_only(self):
        ...
        filtered = self.view.filtered_rows
        bpms = [
            float(r["beatport_bpm"])
            for r in filtered
            if r.get("beatport_bpm", "").strip()
        ]
        self.assertTrue(all(bpm >= 130 for bpm in bpms))
        self.assertIn(130.0, bpms)
        self.assertNotIn(125.0, bpms)
    ...
    def test_key_filter_specific_key(self):
        ...
        filtered = self.view.filtered_rows
        keys = [
            r["beatport_key"] for r in filtered if r.get("beatport_key", "").strip()
        ]
        self.assertTrue(all(key == "C Major" for key in keys))
        self.assertEqual(len(keys), 1)
    ...
    def test_filter_combinations_year_and_bpm(self):
        ...
        for row in filtered:
            if (
                row.get("beatport_year", "").strip()
                and row.get("beatport_bpm", "").strip()
            ):
    ...
    def test_search_filter(self):
        ...
        for row in filtered:
            search_text = "2020"
            matches = (
                search_text in (row.get("original_title", "") or "").lower()
                or search_text in (row.get("original_artists", "") or "").lower()
                or search_text in (row.get("beatport_title", "") or "").lower()
                or search_text in (row.get("beatport_artists", "") or "").lower()
            )
