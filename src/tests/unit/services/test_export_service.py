...
class TestExportService:
    ...
    def test_export_to_csv(self, sample_track_result):
        """Test CSV export."""
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(csv_files) > 0, (
                f"Expected CSV files in {output_dir}, found: {csv_files}"
            )

    def test_export_to_json(self, sample_track_result):
        """Test JSON export."""
        service = ExportService()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                assert (
                    "Title" in data[0]
                    or "title" in data[0]
                    or sample_track_result.title in str(data[0].values())
                )

    def test_export_to_excel(self, sample_track_result):
        """Test Excel export."""
        service = ExportService()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if (
                    "EXPORT_EXCEL_MISSING_DEPENDENCY" in str(e)
                    or "openpyxl" in str(e).lower()
                ):
    ...
    def test_export_multiple_results(
        self, sample_track_result, sample_track_result_unmatched
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            service.export_to_json(
                [sample_track_result, sample_track_result_unmatched], filepath
            )
    ...
    def test_export_to_csv_with_logging(
        self, sample_track_result, mock_logging_service
    ):
    ...
    def test_export_to_csv_custom_delimiter(self, sample_track_result):
    ...
    def test_export_service_init_without_logging(self):
            ...
            try:
                with pytest.raises(Exception):
                    service.export_to_csv(
                        [sample_track_result], "test_output", output_dir=read_only_dir
                    )
            except (PermissionError, OSError):
                # Expected on systems where /root is not writable
                pass

    def test_export_to_json_invalid_data(self, mock_logging_service):
    ...
    def test_export_to_csv_empty_results_handling(self, mock_logging_service):
    ...
    def test_export_service_init_with_logging(self, mock_logging_service):
        """Test ExportService initialization with logging service."""
        service = ExportService(logging_service=mock_logging_service)

        assert service.logging_service is mock_logging_service

    def test_export_to_json_with_special_characters(self, mock_logging_service):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                assert "quotes" in data[0].get("Title", "") or "quotes" in str(
                    data[0].values()
                )
    ...
    def test_export_to_csv_with_none_values(self, mock_logging_service):
    ...
    def test_export_to_json_with_none_values(self, mock_logging_service):
