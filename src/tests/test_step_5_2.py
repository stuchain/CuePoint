...
def test_di_container():
    ...
    assert not container.is_registered(float), (
        "Unregistered service should return False"
    )
...
def test_service_registration():
    ...
    for service_interface in services:
        assert container.is_registered(service_interface), (
            f"{service_interface.__name__} should be registered"
        )
...
def test_service_resolution():
    ...
    assert config.get("MAX_SEARCH_RESULTS") is not None, (
    ...
    assert hasattr(beatport, "cache_service"), (
        "Beatport should have cache service injected"
    )
    assert hasattr(beatport, "logging_service"), (
        "Beatport should have logging service injected"
    )
    ...
    assert hasattr(processor, "beatport_service"), (
        "Processor should have beatport service injected"
    )
    assert hasattr(processor, "matcher_service"), (
        "Processor should have matcher service injected"
    )
    assert hasattr(processor, "logging_service"), (
        "Processor should have logging service injected"
    )
    assert hasattr(processor, "config_service"), (
        "Processor should have config service injected"
    )
...
def test_processor_service_basic():
    ...
    try:
        ...
        print(
            f"⚠ ProcessorService test encountered error (expected for real Beatport search): {e}"
        )
