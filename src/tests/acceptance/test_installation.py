...
def test_macos_dmg_mounts():
    ...
    result = subprocess.run(
        ["hdiutil", "attach", str(dmg_path)], capture_output=True, text=True, timeout=30
    )
...
def test_macos_app_structure():
    ...
    try:
        ...
        with open(app_path / "Contents" / "Info.plist", "rb") as f:
            ...
            assert plist.get("CFBundleIdentifier") == "com.stuchain.cuepoint", (
                "Bundle ID incorrect"
            )
...
def test_windows_installer_runs(clean_install_dir):
    ...
    try:
        ...
        assert (install_path / "CuePoint.exe").exists(), (
            "CuePoint.exe not found after installation"
        )
        ...
        if uninstaller.exists():
            result = subprocess.run(
                [str(uninstaller), "/S"], capture_output=True, text=True, timeout=60
            )
...
@pytest.mark.skipif(
    platform.system() == "Windows", reason="Requires clean VM without Python"
)
